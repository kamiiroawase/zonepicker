...
- 完整适配 edge-to-edge：状态栏、导航栏与键盘 inset 自动处理，宿主 targetSdk 35（Android 15 强制边到边）下内容不被遮挡
...
- **edge-to-edge 验证范围**：inset 处理按宿主 targetSdk 35（Android 15 强制边到边）验证；宿主 targetSdk < 35 且系统低于 Android 15 时，可能出现状态栏区域双倍留白。
- **低版本键盘避让**：API 29 及以下设备的键盘 inset 需要 `adjustResize` 模式，当前未显式声明，个别设备上键盘可能遮挡列表底部。
